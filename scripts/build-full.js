...
runCmd('node', [path.join('scripts', 'generate-icons.js')], 'Generating placeholder icons');
...
runCmd('node', [path.join('scripts', 'setup-android.js')], 'Setting up Capacitor Android project');
...
runCmd('node', [path.join('scripts', 'finalize-android.js')], 'Finalizing Android project (copy icons, patch gradle)');

// Step 6: Patch native manifest
runCmd('node', [path.join('scripts', 'patch-android-native.js')], 'Patching native manifest and build config');

// Step 7: Assemble debug APK
runCmd('node', [path.join('scripts', 'build-android-debug.js')], 'Building debug APK');
